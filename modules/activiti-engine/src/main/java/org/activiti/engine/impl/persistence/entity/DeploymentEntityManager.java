...
public class DeploymentEntityManager extends AbstractManager {
    ...
    public void deleteDeployment(String deploymentId, boolean cascade) {
        ...
        List<Model> models = getDbSqlSession().createModelQueryImpl().deploymentId(deploymentId)
            .list();
        ...
        if (cascade) {
            ...
            for (ProcessDefinition processDefinition : processDefinitions) {
                ...
                getProcessInstanceManager().deleteProcessInstancesByProcessDefinition(
                    processDefinitionId, "deleted deployment", cascade);
            }
        }
        ...
        for (ProcessDefinition processDefinition : processDefinitions) {
            ...
            List<EventSubscriptionEntity> eventSubscriptionEntities = getEventSubscriptionManager()
                .findEventSubscriptionsByTypeAndProcessDefinitionId(null, processDefinitionId,
        ...
        for (ProcessDefinition processDefinition : processDefinitions) {
            ...
            List<Job> timerStartJobs = Context
                .getCommandContext()
                .getJobEntityManager()
                        ...
                        Context
                            ...
                            .dispatchEvent(
                                ActivitiEventBuilder.createEntityEvent(
                                    ActivitiEventType.JOB_CANCELED, timerStartJob, null, null,
                                    processDefinition.getId()));
                    }
            ...
            if (processDefinition.getId().equals(latestProcessDefinition.getId())) {
                ...
                if (previousProcessDefinition != null) {
                    ...
                    ProcessDefinitionEntity resolvedProcessDefinition = Context
                        .getProcessEngineConfiguration()
                    ...
                    // Timer start
                    List<TimerDeclarationImpl> timerDeclarations = (List<TimerDeclarationImpl>) resolvedProcessDefinition
                    ...
                    List<EventSubscriptionDeclaration> signalEventDefinitions = (List<EventSubscriptionDeclaration>) resolvedProcessDefinition
                    ...
                    if (signalEventDefinitions != null) {
                        for (EventSubscriptionDeclaration eventDefinition : signalEventDefinitions) {
                            if (eventDefinition.getEventType().equals("signal")
                                ...
                                subscriptionEntity.setProcessDefinitionId(previousProcessDefinition
                                    .getId());
                                subscriptionEntity.setTenantId(previousProcessDefinition
                                    .getTenantId());
                                ...
                                newSubscription.setProcessDefinitionId(previousProcessDefinition
                                    .getId());
                                newSubscription
                                    .setTenantId(previousProcessDefinition.getTenantId());
    ...
    protected ProcessDefinitionEntity findLatestProcessDefinition(ProcessDefinition processDefinition) {
        ...
        if (processDefinition.getTenantId() != null
            ...
            latestProcessDefinition = Context
                .getCommandContext()
                .getProcessDefinitionEntityManager()
                .findLatestProcessDefinitionByKeyAndTenantId(processDefinition.getKey(),
                    processDefinition.getTenantId());
        } else {
            latestProcessDefinition = Context.getCommandContext()
                .getProcessDefinitionEntityManager()
    ...
    protected ProcessDefinition findNewLatestProcessDefinitionAfterRemovalOf(ProcessDefinition processDefinitionToBeRemoved) {
        ...
        if (processDefinitionToBeRemoved.getTenantId() != null
            && !ProcessEngineConfiguration.NO_TENANT_ID.equals(processDefinitionToBeRemoved
                .getTenantId())) {
    ...
    public List<String> getDeploymentResourceNames(String deploymentId) {
        return getDbSqlSession().getSqlSession().selectList("selectResourceNamesByDeploymentId",
            deploymentId);
    }
