...
public class ClassDelegate extends AbstractBpmnActivityBehavior implements TaskListener,
    ...
    protected ExecutionListener getExecutionListenerInstance() {
        ...
        if (delegateInstance instanceof ExecutionListener) {
            ...
            throw new ActivitiIllegalArgumentException(delegateInstance.getClass().getName()
                                                       + " doesn't implement "
                                                       + ExecutionListener.class + " nor "
                                                       + JavaDelegate.class);
        }
    }
    ...
    public void execute(ActivityExecution execution) throws Exception {
        ...
        if (!isSkipExpressionEnabled
            || (isSkipExpressionEnabled && !SkipExpressionUtil.shouldSkipFlowElement(execution,
                skipExpression))) {
            ...
            if (Context.getProcessEngineConfiguration().isEnableProcessDefinitionInfoCache()) {
                ...
                if (taskElementProperties != null
                    ...
                    if (StringUtils.isNotEmpty(overrideClassName)
                        && overrideClassName.equals(className) == false) {
    ...
    protected ActivityBehavior getActivityBehaviorInstance(ActivityExecution execution) {
        ...
        if (delegateInstance instanceof ActivityBehavior) {
            ...
            return determineBehaviour(new ServiceTaskJavaDelegateActivityBehavior(
                (JavaDelegate) delegateInstance), execution);
        } else {
            throw new ActivitiIllegalArgumentException(delegateInstance.getClass().getName()
                                                       + " doesn't implement "
                                                       + JavaDelegate.class.getName() + " nor "
    ...
    protected ActivityBehavior determineBehaviour(ActivityBehavior delegateInstance,
        ...
        if (hasMultiInstanceCharacteristics()) {
            multiInstanceActivityBehavior
                .setInnerActivityBehavior((AbstractBpmnActivityBehavior) delegateInstance);
    ...
    public static void applyFieldDeclaration(FieldDeclaration declaration, Object target,
        ...
        if (setterMethod != null) {
            try {
                ...
                throw new ActivitiException("Error while invoking '" + declaration.getName()
                                            + "' on class " + target.getClass().getName(), e);
            } catch (IllegalAccessException e) {
                throw new ActivitiException("Illegal acces when calling '" + declaration.getName()
                                            + "' on class " + target.getClass().getName(), e);
            } catch (InvocationTargetException e) {
                throw new ActivitiException("Exception while invoking '" + declaration.getName()
                                            + "' on class " + target.getClass().getName(), e);
            }
        } else {
